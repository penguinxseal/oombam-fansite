...
/* =========================================================
   HOMEPAGE 15.20 — OUR STORY MODAL (isolated add-on)
   Base retained from Moments v15.19
========================================================= */
(() => {
const storyModal=document.getElementById("storyModal"),storyReader=document.getElementById("storyModalReader");
  const storyOpeners=[...document.querySelectorAll("[data-story-open]")],storyClosers=[...document.querySelectorAll("[data-story-close]")],storyChapterButtons=[...document.querySelectorAll("[data-story-chapter]")],storySections=[...document.querySelectorAll("[data-story-section]")];
...
/* =========================================================
   HOMEPAGE 15.31 — ISOLATED DESKTOP STORY CONTROLLER
========================================================= */
(function(){
  const modal=document.getElementById('storyModal');
  if(!modal)return;
  const panel=modal.querySelector('.osd-panel');
  if(!panel)return;
  const chapters=[...panel.querySelectorAll('.osd-chapter')];
  const buttons=[...panel.querySelectorAll('[data-osd-chapter]')];
  function show(id){
    chapters.forEach(ch=>{const on=ch.id===id;ch.hidden=!on;ch.classList.toggle('is-active',on)});
    panel.querySelectorAll('.osd-nav-item').forEach(b=>b.classList.toggle('is-active',b.dataset.osdChapter===id));
...
// =========================================================
// HOMEPAGE 15.44 — MEDIA DETAIL MODAL
// =========================================================
(() => {
  const modal = document.getElementById('mediaDetailModal');
  if (!modal) return;
  const DATA = {
    fulfill: { eyebrow:'Series', title:'Fulfill', image:'https://instagram.fmnl40-2.fna.fbcdn.net/v/t51.82787-15/670839906_17869054245608462_5372531259346478462_n.jpg?stp=dst-jpg_e15_tt6&_nc_cat=108&ig_cache_key=Mzg3ODYzMzc2NDI2MDI3OTU1MTE3ODY5MDU0MjM5NjA4NDYy.3-ccb7-5&ccb=7-5&_nc_sid=58cdad&efg=eyJ2ZW5jb2RlX3RhZyI6IkNMSVBTLnhwaWRzLjEwODAuc2RyLnZpZGVvX2RlZmF1bHRfY292ZXJfZnJhbWUuQzMifQ%3D%3D&_nc_ohc=G4BkmSiMVVsQ7kNvwE8JLqb&_nc_oc=Adp7yFBmPRUHcReYb9tjv5nqxjyeiFAs6IuAD77UinhYiNLZnUeF5kFG8hHWEeGoAd4&_nc_ad=z-m&_nc_cid=0&_nc_zt=23&_nc_ht=instagram.fmnl40-2.fna&_nc_gid=aiHFIGgRAGPD7vjCYXourQ&_nc_ss=7a22e&oh=00_AQKg7_EIigIBDYtfyk20hRxjnPnam4RugbHPzeifG9CpGQ&oe=6AA328EC', description:'Return to the story that started it all — from official episodes and behind-the-scenes material to special content and teasers from the Fulfill era.', credit:'Source: Instagram', creditUrl:'https://www.instagram.com/reel/DXTrNvUkfT_/', actions:[['Explore Fulfill','just-the-two-of-us.html'],['View original post','https://www.instagram.com/reel/DXTrNvUkfT_/']] },
    sakura: {
      eyebrow:'Special Collection',
      title:'When SAKURA Blooms',
      image:'https://instagram.fmnl40-2.fna.fbcdn.net/v/t51.71878-15/727286008_1069176428872871_3360453299667440480_n.jpg?stp=dst-jpg_e15_tt6&_nc_cat=100&ig_cache_key=MzkyNTAwMTY0OTI0ODQ4MTUyMjQ0NzYzNDUyNTI2MjQ4Nzc%3D.3-ccb7-5&ccb=7-5&_nc_sid=58cdad&efg=eyJ2ZW5jb2RlX3RhZyI6IkNMSVBTLnhwaWRzLjY0MC5zZHIudmlkZW9fbmZyYW1lX2NvdmVyX2ZyYW1lLkMzIn0%3D&_nc_ohc=PhQ_HO6gabAQ7kNvwG-1sVc&_nc_oc=AdrM2QmtGm9c4J2k1MnWlzpcn7z7jJtaPx9KlxDQTH9_kyzM6RDbOxbYaGdi0_C5r44&_nc_ad=z-m&_nc_cid=0&_nc_zt=23&_nc_ht=instagram.fmnl40-2.fna&_nc_gid=ciLXm1OV97sXuCLPdQOY2w&_nc_ss=7a22e&oh=00_AQIChYArzyQuVwsxQ0N6fxGN6GfGCrUcPI4Tpsy3dqQr5w&oe=6AA32AFA',
      ...
      creditUrl:'https://www.instagram.com/p/DZ4aDRWRzDy/',
      ...
      clips:[
        ...
        {videoId:'kxlfOFyq_JU', caption:'The relationship handbook: chase me when I say go away. Very efficient communication.'},
        {videoId:'5LWfwCdr188', caption:'A thank-you with suspiciously strong replay value.'},
        {videoId:'nthYOdQzlaY', caption:'Boundaries up, feelings louder. The tension is doing half the dialogue.'},
        {videoId:'k_cfZ42m-Hk', caption:'Tiny sentence, huge emotional damage—in the sweetest way.'},
        {videoId:'E2HkQ9VPsKc', caption:'Domestic softness unlocked. One birthday scene can feed a fandom for months.'},
    ...
    soundtrack: { eyebrow:'Soundtrack', title:'Fulfill OST', image:'https://i.ytimg.com/vi/RHnPq3Z0A8c/hqdefault.jpg', videoId:'RHnPq3Z0A8c', description:'The official Fulfill soundtrack, performed by Oom Eisaya and Bam Saralee.', credit:'Official soundtrack · YouTube', actions:[['Oom Eisaya ↗','https://youtu.be/RHnPq3Z0A8c?si=doOy3CnVHBzP7ASw'],['Bam Saralee ↗','https://youtu.be/CKclkO6HHrY?si=vwnpGo217i5dTE7D']] }
  };
  ...
  const open=(key)=>{ const d=DATA[key]; if(!d)return; modal.dataset.mediaKey=key; const imageWrap=img.parentElement; imageWrap.querySelectorAll('.media-detail-modal__video').forEach(el=>el.remove()); img.hidden=false; img.src=d.image||''; img.alt=d.title||''; if(d.videoId){img.hidden=true;const frame=document.createElement('iframe');frame.className='media-detail-modal__video';frame.src=`https://www.youtube-nocookie.com/embed/${d.videoId}?rel=0`;frame.title=`${d.title} — YouTube`;frame.loading='lazy';frame.allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';frame.allowFullscreen=true;imageWrap.appendChild(frame)} eye.textContent=d.eyebrow; title.textContent=d.title; desc.textContent=d.description; actions.innerHTML=''; (d.actions||[]).forEach(([label,url])=>{const a=document.createElement('a');a.textContent=label;a.href=url;if(/^https?:/.test(url)){a.target='_blank';a.rel='noopener noreferrer'}actions.appendChild(a)}); credit.innerHTML=''; if(d.creditUrl){const a=document.createElement('a');a.href=d.creditUrl;a.target='_blank';a.rel='noopener noreferrer';a.textContent=d.credit+' ↗';credit.appendChild(a)}else credit.textContent=d.credit||''; if(d.note){note.textContent=d.note;note.hidden=false}else{note.textContent='';note.hidden=true} collection.innerHTML=''; if(Array.isArray(d.clips)&&d.clips.length){collection.hidden=false;d.clips.forEach((clip)=>{const card=document.createElement('article');card.className='sakura-clip-card';const player=document.createElement('div');player.className='sakura-clip-card__player';const frame=document.createElement('iframe');frame.src=`https://www.youtube-nocookie.com/embed/${clip.videoId}?rel=0`;frame.title='When SAKURA Blooms — Fulfill clip';frame.loading='lazy';frame.allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';frame.allowFullscreen=true;const caption=document.createElement('p');caption.className='sakura-clip-card__caption';caption.textContent=clip.caption;player.appendChild(frame);card.appendChild(player);card.appendChild(caption);collection.appendChild(card)})}else{collection.hidden=true} modal.classList.add('is-open');modal.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');modal.querySelector('.media-detail-modal__close')?.focus({preventScroll:true})};
  const close=()=>{modal.classList.remove('is-open');modal.setAttribute('aria-hidden','true');delete modal.dataset.mediaKey;modal.querySelectorAll('.media-detail-modal__video').forEach(el=>el.remove());collection.innerHTML='';collection.hidden=true;img.hidden=false;document.body.classList.remove('modal-open')};
