...
(() => {
  ...
  const recoveryRedirectUrl = () => `${window.location.origin}/community.html?recovery=1`;
  ...
  const wirePasswordToggles = (root) => { root.querySelectorAll(".home-password-toggle").forEach(button => button.addEventListener("click", () => { const input=button.closest(".home-password-field")?.querySelector("input"); if(!input)return; const showing=input.type==="text"; input.type=showing?"password":"text"; button.textContent=showing?"Show":"Hide"; button.setAttribute("aria-label",showing?"Show password":"Hide password"); button.setAttribute("aria-pressed",String(!showing)); })); };
